    ...
    <div className="min-h-screen">
      ...
      <main>
        ...
        <div className="max-w-7xl mx-auto px-4 py-20 pb-40">
          {/* Admin Header / Tabs */}
          {isAdmin && (
            <div className="flex flex-wrap items-center gap-4 mb-12 bg-gray-50 dark:bg-gray-900/50 p-2 rounded-2xl w-fit">
              <button
                onClick={() => setAdminTab('properties')}
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${adminTab === 'properties' ? 'bg-white dark:bg-gray-800 shadow-sm text-brand-gold' : 'text-gray-500'}`}
              >
                Gerenciar Imóveis
              </button>
              <button
                onClick={() => setAdminTab('inquiries')}
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${adminTab === 'inquiries' ? 'bg-white dark:bg-gray-800 shadow-sm text-brand-gold' : 'text-gray-500'}`}
              >
                Mensagens / Leads
              </button>
            </div>
